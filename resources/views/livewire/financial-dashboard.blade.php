<div class="max-w-xl mx-auto space-y-4">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6" aria-live="polite" aria-atomic="true">
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    <p class="text-xl font-bold {{ $m['net_profit'] >= 0 ? 'text-emerald-800' : 'text-red-800' }}">
                        Rp{{ number_format($m['net_profit'], 0, ',', '.') }}
                    </p>
                </div>
                <div>
                    <p class="text-xs text-gray-600">Pengeluaran</p>
                    <p class="text-xl font-bold text-gray-900">Rp{{ number_format($m['operational_cost'], 0, ',', '.') }}</p>
                </div>
                <div>
                ...
                <div>
                    <p class="text-xs text-gray-600">Rp / Jam</p>
                    <p class="text-lg font-semibold text-gray-900">
    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6" aria-live="polite" aria-atomic="true">
        <h3 class="text-sm font-semibold text-gray-600 uppercase tracking-wide mb-3">📈 Ringkasan Hari Ini</h3>

        @if ($t['shift_count'] === 0)
            <p class="text-gray-500 text-sm py-2">Belum ada shift hari ini.</p>
        @else
            <div class="grid grid-cols-2 gap-3">
                <div>
                    <p class="text-xs text-gray-600">Gross Revenue</p>
                    <p class="text-xl font-bold text-indigo-800">Rp{{ number_format($t['gross_revenue'], 0, ',', '.') }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-600">Net Profit</p>
                    <p class="text-xl font-bold {{ $t['net_profit'] >= 0 ? 'text-emerald-800' : 'text-red-800' }}">
