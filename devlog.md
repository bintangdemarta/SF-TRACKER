...
### 2026-08-10 — FR-4.2 Historical Reporting & Analytics
- **Service (`app/Services/HistoricalReportService.php`):** `resolvePeriod()` menerjemahkan `daily`/`weekly`/`monthly`/`custom` jadi rentang `CarbonImmutable` konkret (custom divalidasi from≤to, lempar `InvalidArgumentException` kalau kacau). `summary()` sengaja cuma delegasi ke `PerformanceMetricsService::summarizeForPeriod` yang sudah ada (bukan implementasi arithmetic baru) — itu sudah persis mencakup 4 metrik yang diminta (Net Profit, Jarak KM, Efisiensi BBM, rata-rata Cost/KM). `shiftsQuery()` bangun breakdown per-shift pakai correlated subquery (`selectSub`) buat gross_revenue/operational_cost/direct_operational_cost/fuel_liters — satu query total, bukan N+1 per shift. `exportRows()` iterasi lewat `->cursor()->map()` jadi `LazyCollection` — gak pernah nge-load seluruh riwayat shift ke memory sekaligus, seberapa besar pun datanya nanti.
- **Export (`app/Http/Controllers/HistoricalReportExportController.php`):** endpoint `GET /reports/export` (validasi period/format/custom-dates), pakai `rap2hpoutre/fast-excel` (baru di-composer-require, ketemu 2 blocker kecil: `ext-zip` PHP disabled di `php.ini` XAMPP — di-enable — dan versi awal `openspout` butuh PHP 8.3+ padahal server ini PHP 8.2, di-pin ke `fast-excel ^5.4` yang kompatibel). FastExcel/openspout nulis row-by-row langsung ke stream response, format CSV/XLSX ditentukan dari ekstensi filename — satu kode path buat dua format, gak ada logic custom CSV terpisah.
- **UI:** komponen Livewire `HistoricalReport` (`/reports`, nav link "Riwayat") — filter periode, kartu ringkasan (reuse pola visual `financial-dashboard.blade.php`: `x-metric-badge`, warna profit hijau/merah), tabel riwayat shift dengan pagination, tombol export CSV/Excel (link biasa bukan `wire:navigate`, supaya browser treat sebagai download beneran bukan SPA nav).
- **Test:** 28 test baru — `HistoricalReportServiceTest` (resolvePeriod semua cabang, akurasi breakdown per-shift vs perhitungan manual, exclude user/tanggal lain, ordering, pagination, LazyCollection memory-safety), `HistoricalReportExportControllerTest` (auth required, validasi, isi CSV & XLSX diverifikasi beneran dengan parse balik file XLSX-nya pakai `FastExcel->import()`, exclude data user lain), `HistoricalReportTest` (komponen Livewire: default state, summary render, custom-range error handling, export URL). Full suite **183/183 pass**.
- **Larastan:** 2 error nyata di-fix beneran (bukan di-baseline) — `Builder` return type `shiftsQuery()` gak di-generic-annotate (`@return Builder<ShiftSession>`), bikin PHPStan salah infer tipe row hasil `cursor()`. Sisa 8 error baru (3x computed-property `HistoricalReport::$range`, 4x Carbon-cast-recognition di `HistoricalReportService`, 1x `LazyCollection` template-invariance — [keterbatasan terdokumentasi resmi PHPStan](https://phpstan.org/blog/whats-up-with-template-covariant)) masuk kategori debt sistemik yang PERSIS sama polanya dengan yang udah di-baseline di `FinancialDashboard`/`FinancialMetricsService` — di-generate-ulang baseline-nya (total 16→24 error), bukan ditutupi diam-diam.
- **Smoke test browser (Playwright):** login → `/reports` → switch periode (daily/custom) → cek gak ada JS/console error, tombol export cuma muncul pas range valid, screenshot visual dicek manual. Sempat curiga nemu bug (banner "Offline" nongol padahal online) — ternyata false alarm dari cara cek `textContent` (gak peduli CSS `display:none`); pas dicek `isVisible()` beneran, banner-nya memang tersembunyi dengan benar. Dicatat di sini biar gak dianggap ada yang disembunyikan.
- Sekalian: `.env` `APP_URL` yang sempat diubah ke `https://` pas kerjaan PWA (entry sebelumnya) dibalikin ke `http://` lagi (samain sama `.env.example`) karena sempat bikin `SeoTest` gagal — HTTPS lokal dari `mkcert` tetap jalan normal, gak kepengaruh sama `APP_URL`, jadi aman dibalikin.
